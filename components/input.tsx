...
export const InputText = ({placeholder, setText, image, size, value}:Input) => {
  ...
  return (
    <View>
      <View style={[styles.inputView, {borderWidth:(error && error.length>0) ? 1 : 0, borderColor:"#F01F0E"}]}>
        {
          (value?.length > 0 || isFocused) &&
          <ThemedText type='small' style={styles.top}>{placeholder}</ThemedText>
        }
    ...
    <View>
      <View style={[styles.inputView, {borderWidth:(error && error.length>0) ? 1 : 0, borderColor:"#F01F0E"}]}>
      {
        ...
        <View style={{flexDirection:'row', alignItems:'center', paddingVertical:(value?.length > 0 || isFocused) ? 0 : 8}}>
          <TextInput style={styles.input} placeholder={placeholder} secureTextEntry = {showPassword} onChangeText={(text)=>{handleText(text)}} onBlur={validateInput} onFocus={() => setIsFocused(true)}/>
